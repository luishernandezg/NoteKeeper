...
class NavigationTest {
    ...
    fun selectNoteAfterNavigationDrawerChange(){
        ...
        onView(withId(R.id.spinnerCourses)).check(matches(withSpinnerText(Matchers.containsString(note.course?.title))))
        onView(withId(R.id.textNoteTitle)).check(matches(withText(Matchers.containsString(note.title))))
        onView(withId(R.id.textNoteText)).check(matches(withText(Matchers.containsString(note.text))))
    }
}